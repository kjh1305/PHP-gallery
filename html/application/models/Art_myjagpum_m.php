<?
	class Art_myjagpum_m extends CI_Model //모델 클래스 선언
	{
		public function getlist($mno, $text1,$start,$limit)
		{
			if(!$text1)
				$sql="select jagpum.*, member.name9 as member_name from jagpum left join member on jagpum.member_no=member.no9 where jagpum.member_no=$mno order by name limit $start,$limit"; //select문 정의
			else
				$sql="select jagpum.*, member.name9 as member_name from jagpum left join member on 
				jagpum.member_no=member.no9 where jagpum.member_no=$mno and name like '%$text1%' order by name limit $start,$limit";
