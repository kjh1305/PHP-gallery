<?
	class Art_admin_jagpum extends CI_Controller{	//product클래스 선언
		function __construct()		//클래스 생성할 때 초기 설정
	{
		parent::__construct();
		$this->load->database();	//데이터 베이스 연결
		$this->load->model("art_admin_jagpum_m"); //모델 product_m 연결
		...
		public function lists() // 리스트 함수
	{
		...
		if($text1=="")
			$base_url = "/art_admin_jagpum/lists/page"; //$page_segment = 4;
		else
			$base_url = "/art_admin_jagpum/lists/text1/$text1/page"; //$page_segment = 6;
		$page_segment = substr_count(substr($base_url,0,strpos($base_url,"page")), "/")+1;
		$base_url="/~sale9".$base_url;
		
		$config["per_page"] = 5; //페이지당 표시할 line 수
		$config["total_rows"] = $this->art_admin_jagpum_m->rowcount($text1); //전체 레코드 개수 구하기
		...
		$data["list"] = $this->art_admin_jagpum_m->getlist($text1,$start,$limit); //자료읽어 data배열에 저장 
		$this->load->view("art_admin_header"); //상단출력(메뉴)
		$this->load->view("art_admin_jagpum_list",$data); //product_list에 자료전달
	...
	public function add()
	{
			...
			$this->form_validation->set_rules("exp","설명","required|max_length[30]");
			$this->form_validation->set_rules("member_no","작가명","required");
			
			
		if($this->form_validation->run()==FALSE)//검사
		//if(!$_POST) // 목록화면의 추가 버튼을 클릭한 경우
		{
			$data["list"] = $this->art_admin_jagpum_m->getlist_member();
			$this->load->view("art_admin_header");
			$this->load->view("art_admin_jagpum_add",$data); //입력화면포시
		...
		else //입력화면의 저장버튼을 클릭한 경우
		{
			...
			$result=$this->art_admin_jagpum_m->insertrow($data);
		
		redirect("/~sale9/art_admin_jagpum/lists".$text1.$page); // 목록화면 돌아가기
		}
	}
		...
		public function view() // 뷰 함수
	{
		...
		$data["text1"]=$text1;
		$data["page"]=$page;
		$data["row"] = $this->art_admin_jagpum_m->getrow($no);

		$this->load->view("art_admin_header"); //상단출력(메뉴)
		$this->load->view("art_admin_jagpum_view",$data);
		$this->load->view("art_admin_footer"); //하단출력
	}
		...
		public function del()
	{
		...
		$page = array_key_exists("page",$uri_array) ? "/page/".urldecode($uri_array["page"]) : "";
		//$no = $this->uri->segment(4);
		$this->art_admin_jagpum_m->deleterow($no); //삭제
		$this->art_admin_jagpum_m->deletegood($no); //삭제
		redirect("/~sale9/art_admin_jagpum/lists".$text1.$page); // 목록화면 돌아가기
	}
	...
	public function edit()
		{
			...
			//if(!$_POST) // 목록화면의 추가 버튼을 클릭한 경우
			{
				...
				$this->load->view("art_admin_header");
				$data["row"] = $this->art_admin_jagpum_m->getrow($no);
				$this->load->view("art_admin_jagpum_edit",$data); //입력화면포시
				$this->load->view("art_admin_footer");
			}
			else //입력화면의 저장버튼을 클릭한 경우
			{
			...
			if($picname) $data["pic"] = $picname; //파일이름 저장
				$result=$this->art_admin_jagpum_m->updaterow($data,$no);
