<?
		...
		public function lists() // 리스트 함수
	{
		...
		$config["total_rows"] = $this->jangbui_m->rowcount($text1); //전체 레코드 개수 구하기
		...
		$data["list"] = $this->jangbui_m->getlist($text1,$start,$limit); //자료읽어 data배열에 저장 
		...
		public function del()
	{
		...
		//$no = $this->uri->segment(4);
		$this->jangbui_m->deleterow($no); //삭제
		redirect("/~sale9/jangbui/lists".$text1.$page); // 목록화면 돌아가기
	}
		public function add()
	{
			$uri_array=$this->uri->uri_to_assoc(3);
		...
		//if(!$_POST) // 목록화면의 추가 버튼을 클릭한 경우
		{
			...
			$this->load->view("jangbui_add",$data); //입력화면포시
		...
		else //입력화면의 저장버튼을 클릭한 경우
		{
			...
			$data=array(
				...
				"numi9" => $this->input->post("numi",TRUE),
				"numo9" => 0,
	...
	public function edit()
		{
			...
			//if(!$_POST) // 목록화면의 추가 버튼을 클릭한 경우
			{
				...
				$this->load->view("main_header");
				$data["row"] = $this->jangbui_m->getrow($no);
				$this->load->view("jangbui_edit",$data); //입력화면포시
				$this->load->view("main_footer");
			}
			else //입력화면의 저장버튼을 클릭한 경우
			{
	...
	{
			$config['upload_path'] = './jangbui_img'; //저장할 경로
